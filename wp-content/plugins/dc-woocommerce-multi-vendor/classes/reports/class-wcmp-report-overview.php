...
class WCMp_Report_Overview extends WC_Admin_Report {
    ...
    private function query_report_data() {
        ...
        for ($date = $start_date; $date <= strtotime('+1 day', $end_date); $date = strtotime('+1 day', $date)) {
            ...
            $args = apply_filters('wcmp_vendor_report_overview_query_args', array(
            ...
            $orders = apply_filters('wcmp_filter_orders_report_overview', $qry->get_posts(), $this);
            ...
            $chart_data_net_earnings[] = wcmpArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'net_earnings' => $net_earnings));
        }
        ...
        $this->report_data->order_counts = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        'ID' => array(
                            'type' => 'post_data',
        ...
        $this->report_data->coupons = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
        ...
        $this->report_data->order_items = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        '_qty' => array(
                            'type' => 'order_item_meta',
        ...
        $this->report_data->refunded_order_items = absint(
                $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        '_qty' => array(
                            'type' => 'order_item_meta',
         ...
         */
        $this->report_data->orders = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        '_order_total' => array(
                            'type' => 'meta',
         ...
         */
        $this->report_data->full_refunds = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        '_order_total' => array(
                            'type' => 'parent_meta',
         ...
         */
        $this->report_data->partial_refunds = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        'ID' => array(
                            'type' => 'post_data',
         ...
         */
        $this->report_data->refund_lines = (array) $WCMp->report->get_order_report_data(
                        array(
                    'data' => array(
                        'ID' => array(
                            'type' => 'post_data',
        ...
        $this->report_data = apply_filters('wcmp_admin_vendor_report_data', $this->report_data);
    }
    ...
    public function get_chart_legend() {
        ...
        switch ($this->chart_groupby) {
            case 'day':
                ...
                $average_total_sales_title = sprintf(
                        __('%s average gross daily sales', 'woocommerce'), '<strong>' . wc_price($data->average_total_sales) . '</strong>'
                );
                /* translators: %s: average sales */
                $average_sales_title = sprintf(
                        __('%s average net daily sales', 'woocommerce'), '<strong>' . wc_price($data->average_sales) . '</strong>'
                );
                break;
            case 'month':
            default:
                /* translators: %s: average total sales */
                $average_total_sales_title = sprintf(
                        __('%s average gross monthly sales', 'woocommerce'), '<strong>' . wc_price($data->average_total_sales) . '</strong>'
                );
                /* translators: %s: average sales */
                $average_sales_title = sprintf(
                        __('%s average net monthly sales', 'woocommerce'), '<strong>' . wc_price($data->average_sales) . '</strong>'
                );
                break;
        }
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s gross sales in this period', 'woocommerce'), '<strong>' . wc_price($data->total_sales) . '</strong>'
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s net sales in this period', 'woocommerce'), '<strong>' . wc_price($data->net_sales) . '</strong>'
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s orders placed', 'woocommerce'), '<strong>' . $data->total_orders . '</strong>'
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s items purchased', 'woocommerce'), '<strong>' . $data->total_items . '</strong>'
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    _n('%1$s refunded %2$d order (%3$d item)', '%1$s refunded %2$d orders (%3$d items)', $this->report_data->total_refunded_orders, 'woocommerce'), '<strong>' . wc_price($data->total_refunds) . '</strong>', $this->report_data->total_refunded_orders, $this->report_data->refunded_order_items
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s charged for shipping', 'woocommerce'), '<strong>' . wc_price($data->total_shipping) . '</strong>'
            ),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(
                    __('%s worth of coupons used', 'woocommerce'), '<strong>' . wc_price($data->total_coupons) . '</strong>'
            ),
